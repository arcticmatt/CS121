...
CREATE FUNCTION tree_depth() RETURNS INTEGER BEGIN
    ...
    -- Insert the root of the tree into emp_tree
    INSERT INTO emp_tree
        SELECT emp_id
        FROM employee_adjlist
        WHERE manager_id IS NULL;

    -- Then, while emp_tree is being updated, we will insert employees from
    -- emp_adjlist if their manager is in emp_tree and they are not already
    -- contained in emp_tree. This basically inserts the tree into the
    -- emp_tree table one level at a time. So then, we just need to count
    ...
    WHILE ROW_COUNT() > 0 DO
        SET height = height + 1;
        INSERT INTO emp_tree SELECT emp_id FROM employee_adjlist
            WHERE manager_id IN (SELECT emp_id FROM emp_tree)
            AND emp_id NOT IN (SELECT emp_id FROM emp_tree);
    END WHILE;
    RETURN height;
